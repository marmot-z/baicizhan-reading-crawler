...
;(function($, window, document, undefined) {
	...
	$.fn.initAudioPlayer = function(options = {}) {
		// 遍历处理audio
		this.each(function() {
			...
			// 添加播放器盒子
			var $player = $(`<div class="ppq-audio-player">
						${$('<div>').append($this.eq(0).clone()).html()}
						<img id="changeSpeedImg" src="../../../../assets/bootstrap-5.0.2-dist/icons/soundwave.svg"                             
                            style="float: left; margin-top: 40px; width: 32; height: 32; cursor: pointer;"/>
						<div class="play-pause-btn">
							<a href="javascript: void(0);" class="play-pause-icon"></a>
						</div>
					</div>`),
			...
			function initAudioEvents() {
				...
				audioEle.addEventListener('timeupdate', function() {
                    ...
                    if (options.onTimeUpdate) options.onTimeUpdate(audioEle.currentTime);
				});
			...
			function addChangeSpeedEvent() {
				$player.find('#changeSpeedImg')
					.on('click', (e) => {
						e.preventDefault();
						let a = $player.find('audio')[0];
						a.pause();
						a.src = `https:${q.next().audio_url}`;
						a.load();
						a.play();
					});
			}
